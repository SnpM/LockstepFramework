...
namespace Lockstep
{
    ...
    public  static partial class LSEditorUtility
    {
        ...
        public static void EnumField<T> (string content, ref T value) where T: struct, IComparable, IConvertible, IFormattable
        {
            ...
            if (value.GetType ().IsEnum == false) throw new ArgumentException ("T must be enumerated type");
            ...
            SerializeAction<T> elementAction)
        {
            if (showEnumArray = EditorGUILayout.Foldout (showEnumArray, content))
            {
                ...
                if (values.Length != size) Array.Resize (ref values, size);

                BeginIndent ();
                if (showElements == null) showElements = new bool[values.Length];
                else if (showElements.Length != values.Length) Array.Resize<bool> (ref showElements, values.Length);
        ...
        {
            GenerateEnum<TData> (enumName, data.Data, (item) => item.Name, (item) => (int)item.MappedCode);
        }
        public static void GenerateEnum<TData> (
            string enumName,
            TData[] data,
        ...
        {
            string generationFolder = Application.dataPath + "/" + LSDatabaseManager.DATABASE_FOLDER + "/Generated/";
            ...
            */
            ...
            //Add default
            newEnum.Members.Add(new CodeMemberField {Name = "None", InitExpression = new CodePrimitiveExpression (0)});
            enumValues.Add(0);
            enumNames.Add("None");
            for (int i = 0; i < data.Length; i++) {
                TData item = data [i];
        ...
        public static IEnumerable<TAttribute> GetPropertyAttributes<TAttribute> (SerializedProperty prop) {
            ...
            for (int i = 0; i < attributes.Length; i++) yield return (TAttribute)attributes[i] ;
        }
