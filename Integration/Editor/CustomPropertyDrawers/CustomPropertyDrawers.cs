...
namespace Lockstep
{
	//TODO: Organize these classes into different files
	...
	public class EditorVector3d : PropertyDrawer
	{
		//Thanks stranger! (https://gamedev.stackexchange.com/a/123609/57906)
		...
		public override void OnGUI (Rect position, SerializedProperty property, GUIContent label)
		{
			...
			float half = contentPosition.width / 3f;
			GUI.skin.label.padding = new RectOffset (3, 3, 6, 6);
			...
			{
				EditorGUI.BeginChangeCheck ();
				var newVal = FixedNumberField (contentPosition, new GUIContent ("Z"), Y.longValue);
				if (EditorGUI.EndChangeCheck ())
					Y.longValue = newVal;
			}
			EditorGUI.EndProperty ();
