...
namespace Lockstep
{
	public class CollisionPair
	{
		...
		public void Initialize (LSBody b1, LSBody b2)
		{
			...
			if (!PhysicsManager.SimulatePhysics)
				DoPhysics = false;
			else
		...
		public void CheckAndDistributeCollision ()
		{
			...
			if (SamePartition && CheckCollision ()) {
				SamePartition = false;
				if (!IsColliding) {
		...
		public bool CheckCollision ()
		{
			...
			if (!(Body1.PositionChangedBuffer || Body1.RotationChangedBuffer || Body2.PositionChangedBuffer || Body2.RotationChangedBuffer)) {
				return IsColliding;
			}
